...
contract Pyramid is ColorToken{
	...
	string constant public name = "Clocks";
	...
	function devPull() public{
		require(msg.sender == pineapples && devPot > 0);
		(bool success, ) = pineapples.call{value:devPot}("");
	...
	function buy(address addr, uint _red, uint _green, uint _blue) public payable returns(uint createdBonds){
		...
		if( (now - genesis) < 86400*30*6 ){
			uint devSlice = numEther * 5 / 1000;
			numEther = numEther - devSlice;
			devPot += devSlice;
		}
	...
	function sell(uint256 amount) internal returns(uint eth, uint resolves, uint initialInput){
		...
		if ( dissolvingResolves > 0 ){
			...
			resolveFee = _sellSideDivs * scaleFactor;
